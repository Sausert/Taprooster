...
// ── Staafdiagram component ──
function TapBarChart({ leaderboard }: { leaderboard: any[] }) {
  const max = Math.max(...leaderboard.map(l => l.taps_this_year || 0), 1);
  return (
    <div style={{ marginTop:8 }}>
      {leaderboard.slice(0, 10).map((lb, i) => {
        const pct = Math.round(((lb.taps_this_year || 0) / max) * 100);
        const firstName = lb.full_name?.split(" ")[0] || "?";
        return (
          <div key={lb.id} style={{ marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
              <span style={{ fontSize:12, color:"#e8e0ff", fontWeight:600 }}>{firstName}</span>
              <span style={{ fontSize:12, fontFamily:"monospace", color:"#00e5c3" }}>{lb.taps_this_year || 0}x</span>
            </div>
            <div style={{ background:"#2e2a4a", borderRadius:4, height:8, overflow:"hidden" }}>
              <div style={{
                height:"100%", borderRadius:4,
                width:`${pct}%`,
                background: pct > 75 ? "linear-gradient(90deg,#00e5c3,#00b89c)"
                           : pct > 40 ? "linear-gradient(90deg,#ffb547,#e09030)"
                           : "linear-gradient(90deg,#5a4a9e,#3b2f6e)",
...
export default function AdminClient({ shifts: initialShifts, profiles: initialProfiles, leaderboard, publishedShifts: initialPublished }: {
  ...
  function getHealthColor(shift:any) {
    const n = (shift.assignments||[]).filter((a:any)=>a.status!=="declined").length;
  ...
  async function saveTapper() {
    ...
    if(res.ok) setProfiles(ps=>ps.map(p=>p.id===editingTapper.id?{...p,...editForm}:p));
    setSavingTapper(false); setEditingTapper(null);
  }

  // Tapper handmatig toevoegen
  async function handleAddTapper(shiftId:string, userId:string) {
    ...
    if(res.ok) {
      ...
      setPublished(updateList); setConceptShifts(updateList);
    }
  ...
  async function handlePublish() {
    setPublishing(true);
    await fetch("/api/schedule/publish",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({months:getMonths(),message:publishMsg})});
    setPublishing(false); alert("🚀 Rooster gepubliceerd!"); setConceptShifts([]); setPublishMsg("");
  }
  ...
  // ── ShiftCard ── (werkt voor zowel concept als published)
  const ShiftCard = ({ shift, source }: { shift: any; source: "concept"|"published" }) => {
    ...
    const accentColor = shift.type === "feestje" ? "#ffb547" : "#00e5c3";
        ...
        {isEditing ? (
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6,marginLeft:10}}>
              ...
              {source==="concept" && <button style={{...s.iconBtn,color:"#ff4f6d"}} onClick={()=>setConceptShifts(cs=>cs.filter(x=>x.id!==shift.id))}>🗑</button>}
              {source==="published" && <button style={{...s.iconBtn,color:"#ff4f6d"}} onClick={()=>handleDeleteShift(shift.id,"published")}>🗑</button>}
            </div>
          </div>
        )}
      </div>
      ...
      <div style={s.content}>
        ...
        {tab==="health" && (
          <>
                ...
                return (
                  <div key={shift.id} style={s.healthRow}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <p style={{fontSize:11,color}}>{assigned}/{shift.max_tappers} bezet · {(shift.assignments||[]).filter((a:any)=>a.status==="confirmed").length} bevestigd</p>
                    </div>
            ...
            <div style={s.card}>
              <TapBarChart leaderboard={leaderboard}/>
            </div>
          </>
        )}
        ...
        {tab==="rooster" && (
          <>
            ...
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {/* Fix: gebruik aparte border properties i.p.v. shorthand mix */}
              {(["published","concept"] as const).map(v=>(
                <button key={v} onClick={()=>setRosterView(v)} style={{
                  flex:1, padding:"10px", borderRadius:10,
                  fontFamily:"'Exo 2', sans-serif", fontWeight:700, fontSize:13, cursor:"pointer",
                  background: rosterView===v ? "rgba(0,229,195,0.1)" : "#221f38",
                  color: rosterView===v ? "#00e5c3" : "#8b80b0",
            ...
            {rosterView==="concept" && (
              <>
                ...
                <div style={s.card}>
                  {/* Fix: gebruik borderWidth/Style/Color i.p.v. border shorthand in chipActive */}
                  <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
                    {PERIOD_OPTIONS.map(opt=>(
                      <div key={opt.value} onClick={()=>setPeriod(opt.value)} style={{
                        ...
                        background: period===opt.value ? "rgba(0,229,195,0.1)" : "#221f38",
                        color: period===opt.value ? "#00e5c3" : "#8b80b0",
                        borderWidth:1, borderStyle:"solid",
                        borderColor: period===opt.value ? "#00e5c3" : "#2e2a4a",
                      }}>
                        {opt.label}
                      </div>
                    ))}
