...
const TABS: Array<{ id: AdminTab; icon: React.ReactNode; label: string }> = [
  { id: "health",      icon: <IconHealth />,       label: "Status" },
  { id: "tappers",     icon: <IconTappers />,      label: "Tappers" },
  { id: "rooster",     icon: <IconRoosterAdmin />, label: "Rooster" },
  { id: "berichten",   icon: <IconMessages />,     label: "Berichten" },
  { id: "uitnodiging", icon: <IconInvite />,        label: "Uitnodiging" },
];
...
export default function AdminClient({
  ...
  async function saveShiftEdit(shift: Shift & Record<string, unknown>) {
    ...
    const trimTime = (t: unknown) => typeof t === "string" ? t.slice(0, 5) : t;
    const res = await fetch(`/api/shifts/${shift.id}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ title: shift.title, start_time: trimTime(shift.start_time), end_time: trimTime(shift.end_time), max_tappers: shift.max_tappers }) });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
